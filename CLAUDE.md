...
A skeleton: the approval and execution flow is not designed yet. Out of scope so far: approval, execution stages, BAST, payment, notifications.
...
- **Files.** Routes in `routes/web.php` (`work-orders.*`, `work-orders.export`, `work-orders.restore`, `work-orders.transitions.store`); `app/Http/Controllers/WorkOrders/`, `app/Http/Requests/WorkOrders/`, `WorkOrderResource`; pages in `resources/js/pages/work-orders/`, components in `resources/js/components/work-orders/`, types in `types/workOrder.ts`, badge classes in `lib/workOrderStatus.ts`.
